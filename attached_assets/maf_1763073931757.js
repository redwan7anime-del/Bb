...
const ROLE_DESCS = {
  ...
  'عازف':    '🎻 تملك قوة ربط مصيرين معاً — في الليل الأول فقط أرسل رقمين بالصيغة: 3+4\nإذا مات أي منهما مات الآخر فوراً، طوال بقية اللعبة.',
  'البارون الفقير': '🥸 معك 20 بنساً — كل ليلة تستخدم قدرتك تخسر 5 بنسات.\nقدرتك: تلغي تطبيق دور لاعب واحد في الليل.\nعندما تنتهي بنساتك تصبح بارون فقير بلا قدرة.',
...
function assignRoles(players){
  ...
  // عدد المافيا: 1 دائماً، 2 فوق 6 لاعبين
  const mCount = n > 6 ? 2 : 1;
  let mi = 0;
  while(mi < mCount){
    const r = Math.floor(Math.random()*n);
    if(roles[r] === 'مواطن'){ roles[r] = 'مافيا'; mi++; }
  }

  // بناء قائمة الأدوار المدنية الخاصة حسب العتبات
  const specialRoles = [];
  specialRoles.push('طبيب');                           // دائماً
  specialRoles.push('محقق');                           // دائماً
  if(n > 4)  specialRoles.push('قناص');
  if(n > 5)  specialRoles.push('مهرج');
  if(n > 5)  specialRoles.push('جلاد');
  if(n > 7)  specialRoles.push('نهاب');
  if(n > 7)  specialRoles.push('عازف');
  if(n > 8)  specialRoles.push('البارون الفقير');
  if(n > 8)  specialRoles.push('عالم مجنون');
  if(n > 9)  specialRoles.push('SCP-096');
  if(n > 12) specialRoles.push('دراكولا');

  // خلط عشوائي للأدوار الخاصة (حتى لا يكون الترتيب ثابتاً)
  for(let j = specialRoles.length-1; j > 0; j--){
    const k = Math.floor(Math.random()*(j+1));
    [specialRoles[j], specialRoles[k]] = [specialRoles[k], specialRoles[j]];
  }

  // الحصول على مواقع المدنيين وخلطها
  const civs = roles.map((r,idx) => r==='مواطن' ? idx : -1).filter(x => x >= 0);
  for(let j = civs.length-1; j > 0; j--){
    const k = Math.floor(Math.random()*(j+1));
    [civs[j], civs[k]] = [civs[k], civs[j]];
  }

  // توزيع الأدوار الخاصة على المدنيين
  for(let i = 0; i < specialRoles.length && i < civs.length; i++){
    roles[civs[i]] = specialRoles[i];
  }

  players.forEach((p, i) => {
    p.role = roles[i];
    if(p.role === 'البارون الفقير') p.baronPence = 20;
  });
}
...
async function resolveNight(game){
  ...
  // البارون الفقير — collect blocked players (their night action is cancelled)
  const baronBlocked = new Set();
  for(const [uid,act] of Object.entries(game.actionQueue)){
    const a=byId[uid]; if(!a||!a.alive) continue;
    if(a.role==='البارون الفقير' && act.target) baronBlocked.add(act.target);
  }
  // Doctor
  for(const [uid,act] of Object.entries(game.actionQueue)){
    const a=byId[uid]; if(!a||!a.alive) continue;
    if(baronBlocked.has(uid)) continue;
...
async function dm(userID, text){
  ...
  const msg = typeof text === 'string' ? {body: text} : text;
  const ok = await sendMessageWithRetry(msg, target, 3, 2000);
  if(!ok) console.warn(`[dm] failed after retries for user ${userID}`);
  await sleep(800);
}

function buildMentionedPrompt(header, players, footer){
  let body = header;
  const mentions = [];
  players.forEach((p, i) => {
    const prefix = `${NUM_EMOJIS[i]||`${i+1}.`} `;
    mentions.push({ tag: p.name, id: p.userID, fromIndex: body.length + prefix.length });
    body += prefix + p.name + '\n';
  });
...
login(CREDENTIALS, (err, a)=>{
  ...
  function nightActorsExpected(game){
    ...
    for(const p of game.players){
      ...
      if(p.role === 'البارون الفقير' && (p.baronPence || 0) <= 0) continue;
  ...
  async function startNight(gameId){
    ...
    for(const p of alive){
      try {
        ...
        if(p.role === 'قناص'){
          if(!game.sniperUsed){
            const shootTargets = alive.filter(x => x.userID !== p.userID);
            await dm(p.userID, buildMentionedPrompt(
              `━━━━━━━━━━━━━━━━\n🏹 الليلة ${game.round} — القناص\n━━━━━━━━━━━━━━━━\nمن تستهدف برصاصتك الوحيدة؟\n`,
              shootTargets,
              `\n\nأرسل الرقم أو الاسم — رصاصة واحدة فقط طوال اللعبة:`
            ));
          }
        ...
        if(p.role === 'البارون الفقير'){
          if((p.baronPence || 0) <= 0){
            await dm(p.userID, `🥸 بارون فقير — نفدت بنساتك، لا قدرة لديك هذه الليلة.`);
          } else {
            const baronTargets = alive.filter(x => x.userID !== p.userID);
            await dm(p.userID, buildMentionedPrompt(
              `━━━━━━━━━━━━━━━━\n🥸 البارون الفقير — الليلة ${game.round}\n━━━━━━━━━━━━━━━━\n💰 بنساتك: ${p.baronPence}\nاختر من تلغي دوره الليلة:\n`,
              baronTargets,
              `\n\nأرسل الرقم أو الاسم:`
            ));
          }
        ...
        if(p.role === 'عالم مجنون'){
          ...
          if(killUsed && reviveUsed){ continue; }
          if(!killUsed){
            const aliveOthers = alive.filter(x => x.userID !== p.userID);
            await dm(p.userID, buildMentionedPrompt(
              `🧪 العالم المجنون — الجولة ${game.round}\n━━━━━━━━━━━━━━━━\n☠️ جرعة الموت جاهزة — أرسل رقم هدفك:\n`,
              aliveOthers,
              `\n━━━━━━━━━━━━━━━━`
            ));
          } else {
            ...
            if(deadPlayers.length > 0){
              await dm(p.userID, buildMentionedPrompt(
                `🧪 العالم المجنون — الجولة ${game.round}\n━━━━━━━━━━━━━━━━\n💉 كبسولة الإحياء متاحة — أرسل رقم من تريد إعادته:\n`,
                deadPlayers,
                `\n━━━━━━━━━━━━━━━━`
              ));
            } else {
              await dm(p.userID, `🧪 العالم المجنون — الجولة ${game.round}\n━━━━━━━━━━━━━━━━\n💉 كبسولة الإحياء: لا أموات حالياً ✖\n━━━━━━━━━━━━━━━━`);
            }
          }
          continue;
        }
        const mafiaPartner = p.role === 'مافيا'
          ...
          : null;
        const targets = alive.filter(x =>
          x.userID !== p.userID && (!mafiaPartner || x.userID !== mafiaPartner.userID)
        );
        if(p.role === 'SCP-096'){
          await dm(p.userID, buildMentionedPrompt(
            `━━━━━━━━━━━━━━━━\n☣️ SCP-096 — الليلة ${game.round}\n━━━━━━━━━━━━━━━━\nمن سيرى وجهك الليلة؟\n`,
            targets, `\n\nأرسل الرقم أو الاسم:`
          ));
        } else if(p.role === 'نهاب'){
          await dm(p.userID, buildMentionedPrompt(
            `━━━━━━━━━━━━━━━━\n🦹 الليلة ${game.round} — النهاب\n━━━━━━━━━━━━━━━━\nمن تسرق دوره الليلة؟\n`,
            targets, `\n\nأرسل الرقم أو الاسم:`
          ));
        } else if(p.role === 'دراكولا'){
          await dm(p.userID, buildMentionedPrompt(
            `━━━━━━━━━━━━━━━━\n🧛 الليلة ${game.round} — دراكولا\n━━━━━━━━━━━━━━━━\nمن تحول إلى مصاص دماء؟\n`,
            targets, `\n\nأرسل الرقم أو الاسم:`
          ));
        } else {
          await dm(p.userID, buildMentionedPrompt(
            `━━━━━━━━━━━━━━━━\n🌑 الليلة ${game.round} — ${p.role}\n━━━━━━━━━━━━━━━━\n`,
            targets, `\n\nأرسل الرقم أو الاسم:`
          ));
        }
      } catch(err){
    ...
    async function doNightResolution(){
      ...
      // Send القناص wasted-bullet notice privately only
      for(const d of deaths){
        if(d.bySniper && d.sniperID && d.wasted){
    ...
    for(const p of alive){
      try {
        if((game.blockedVoters || []).includes(p.userID)){
          ...
          const voteTargets = alive.filter(x => x.userID !== p.userID);
          await dm(p.userID, buildMentionedPrompt(`من سيقابل المشنقة ؟\n`, voteTargets, ''));
        }
      } catch(err){
            ...
            if(game.phase === 'night'){
              ...
              if(player.role === 'عالم مجنون'){
                ...
                if(!game.madScientistKillUsed){
                  ...
                  if(deadNow.length > 0){
                    dm(sender, buildMentionedPrompt(`💉 كبسولة الإحياء ظهرت الآن — أرسل رقم من تريد إعادته:\n`, deadNow, ''));
                  }
                  continue;
                }
              ...
              // البارون الفقير — cancel a player's night action
              if(player.role === 'البارون الفقير'){
                if((player.baronPence || 0) <= 0){ continue; }
                if(game.actionQueue[sender]){ continue; }
                const baronAlive = game.players.filter(p=>p.alive && p.userID !== sender);
                const tid = resolveTarget(text, baronAlive);
                if(!tid){ dm(sender, STR.invalidTarget); continue; }
                player.baronPence = (player.baronPence || 20) - 5;
                game.actionQueue[sender] = {target: tid};
                const targetPlayer = game.players.find(p=>p.userID===tid);
                dm(sender, `🥸 تم — ألغيت دور ${targetPlayer?targetPlayer.name:'الهدف'}. بقي لك ${player.baronPence} بنس.`);
                await dm(tid, `خذ 5بنس واسكت 🥸`);
                saveGame(game);
                checkEarlyNight(game.id);
                continue;
              }
